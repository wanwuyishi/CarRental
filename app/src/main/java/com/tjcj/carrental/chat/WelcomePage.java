...
//		}
		...
		String hasUserPwd = user1[1];
		String hasUser=user1[2];
		boolean hasLogin = MyApplication.getHasLogin(this);
		if(hasUserName != null &&hasUserPwd!=null&&hasUser!=null&& hasLogin == true){
			UserUtil.user=(User)SerializeUtils.unSerialize(hasUser);
	...
	private GotyeDelegate mDelegate = new GotyeDelegate(){
		...
		public void onLogin(int code, GotyeUser user) {
			...
			if (code == GotyeStatusCode.CodeOK //0
				...
				String hasUser=user1[2];
				LoginPage.saveUser(WelcomePage.this, true, (User)SerializeUtils.unSerialize(hasUser));
